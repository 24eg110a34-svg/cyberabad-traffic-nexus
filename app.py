...
try:
    from cv_processor import generate_camera_feed
    CV_ENABLED = True
except ImportError:
    CV_ENABLED = False
    print("CV module not available")
...
def api_cctv_analyze(camera_id):
    ...
    v_count = random.randint(5, 25)
    h_viol = random.randint(0, 3)
    m_viol = random.randint(0, 2)
    s_viol = random.randint(0, 4)
    risk = (h_viol * 15) + (m_viol * 20) + (s_viol * 10)
    risk_lvl = "low" if risk < 20 else "medium" if risk < 40 else "high"
...
def api_cctv_all_cameras():
    ...
    for i in range(1, 5):
        cameras.append({
            ...
            "violations": {"helmet": random.randint(0, 3), "mobile": random.randint(0, 2), "over_speed": random.randint(0, 4)},
